...
export async function POST(request: NextRequest) {
  try {
      ...
      try {
        ...
        await prisma.lead.create({
          data: {
            ...
            source: (row.source as any) || "MANUAL_OUTREACH",
            sourceWebsite: row.sourceWebsite || null,
            status: (row.status as any) || "NEW",
            temperature: (row.temperature as any) || "WARM",
            grade: (row.grade || row.leadScore as any) || "C",
            interestProducts: row.interestProducts || row.productInterest || null,
            inquiryContent: row.inquiryContent || null,
    ...
    await createActivityLog({
      action: "导入",
      entityType: "线索",
      description: `导入线索: 成功 ${success} 条, 跳过 ${skipped} 条, 失败 ${failed} 条`,
    });
